(set-logic HORN)

(declare-datatypes ((|internal_node| 0)) (((|internal_node|  (|internal_node::next| Int)))))
(declare-datatypes ((|AddrRange| 0)) (((|AddrRange|  (|AddrRangeStart| Int) (|AddrRangeSize| Int)))))
(declare-datatypes ((|sll| 0)) (((|sll|  (|sll::next| Int) (|sll::shared| Int)))))
(declare-datatypes ((|HeapObject| 0)) (((|O_Int|  (|getInt| Int)) (|O_UInt|  (|getUInt| Int)) (|O_Addr|  (|getAddr| Int)) (|O_AddrRange|  (|getAddrRange| AddrRange)) (|O_internal_node|  (|getinternal_node| internal_node)) (|O_sll|  (|getsll| sll)) (|defObj| ))))
(declare-datatypes ((|Heap| 0)) (((|HeapCtor|  (|HeapSize| Int) (|HeapContents| (Array Int HeapObject))))))

(declare-fun |inv_main_16| ( Heap Int Int Int Int Int ) Bool)
(declare-fun |inv_main_18| ( Heap Int Int Int Int ) Bool)
(declare-fun |inv_main2401_5_15| ( Heap Int Int Int Int Int ) Bool)
(declare-fun |inv_main2399_1_14| ( Heap Int Int Int Int Int ) Bool)
(declare-fun |inv_main2455_5| ( Heap ) Bool)
(declare-fun |inv_main2457_5_20| ( Heap Int Int Int ) Bool)
(declare-fun |inv_main2401_5| ( Heap Int Int Int ) Bool)
(declare-fun |inv_main_3| ( Heap Int Int ) Bool)
(declare-fun |inv_main_8| ( Heap Int Int Int ) Bool)
(declare-fun |inv_main2425_5| ( Heap Int Int Int Int ) Bool)
(declare-fun |inv_main2408_5_5| ( Heap Int Int Int ) Bool)
(declare-fun |inv_main2408_5| ( Heap Int ) Bool)
(declare-fun |CHC_COMP_FALSE| ( ) Bool)
(declare-fun |inv_main_13| ( Heap Int Int Int Int ) Bool)
(declare-fun |inv_main2458_5_23| ( Heap Int Int Int ) Bool)
(declare-fun |inv_main2406_1_4| ( Heap Int Int Int ) Bool)

(assert
  (forall ( (A Heap) ) 
    (=>
      (and
        (= A (HeapCtor 0 ((as const (Array Int HeapObject)) defObj)))
      )
      (inv_main2455_5 A)
    )
  )
)
(assert
  (forall ( (A Int) (B sll) (C Heap) (D Int) (E Int) (F Int) (G Int) (H Heap) (I Int) (J Int) (K Int) (L Int) (M Int) (N Heap) ) 
    (=>
      (and
        (inv_main2425_5 N M L K J)
        (let ((a!1 (ite (and (not (<= J 0)) (>= (HeapSize N) J))
                (select (HeapContents N) J)
                defObj))
      (a!4 (HeapCtor (+ 1 (HeapSize H))
                     (store (HeapContents H) (+ 1 (HeapSize H)) (O_sll B)))))
(let ((a!2 (O_sll (sll (|sll::next| (getsll a!1)) L))))
(let ((a!3 (ite (and (not (<= J 0)) (>= (HeapSize N) J))
                (HeapCtor (HeapSize N) (store (HeapContents N) J a!2))
                N)))
  (and (not (= I 0))
       (= G M)
       (= F L)
       (= E K)
       (= D J)
       (= A (+ 1 (HeapSize H)))
       (= H a!3)
       (= C a!4)
       ((_ is O_sll) a!1)))))
      )
      (inv_main2401_5_15 C G F E D A)
    )
  )
)
(assert
  (forall ( (A Int) (B sll) (C Heap) (D Int) (E Int) (F Int) (G Int) (H Int) (I Heap) (J Int) (K Int) (L Int) (M Int) (N Int) (O Heap) ) 
    (=>
      (and
        (inv_main_18 O N M L K)
        (let ((a!1 (ite (and (not (<= K 0)) (>= (HeapSize O) K))
                (select (HeapContents O) K)
                defObj))
      (a!2 (HeapCtor (+ 1 (HeapSize I))
                     (store (HeapContents I) (+ 1 (HeapSize I)) (O_sll B)))))
  (and (= D (|sll::next| (getsll a!1)))
       (= A (+ 1 (HeapSize I)))
       (not (= J 0))
       (= H N)
       (= G M)
       (= F L)
       (= E K)
       (= C a!2)
       (= I O)
       ((_ is O_sll) a!1)))
      )
      (inv_main2401_5_15 C H G F D A)
    )
  )
)
(assert
  (forall ( (A Int) (B internal_node) (C Heap) (D Int) (E Int) (F Int) (G Heap) (H Int) (I Int) (J Int) (K Heap) ) 
    (=>
      (and
        (inv_main_3 K J I)
        (let ((a!1 (ite (and (not (<= I 0)) (>= (HeapSize K) I))
                (select (HeapContents K) I)
                defObj))
      (a!2 (HeapCtor (+ 1 (HeapSize G))
                     (store (HeapContents G)
                            (+ 1 (HeapSize G))
                            (O_internal_node B)))))
  (and (= E I)
       (not (= H 0))
       (= F J)
       (= D (|internal_node::next| (getinternal_node a!1)))
       (= A (+ 1 (HeapSize G)))
       (= G K)
       (= C a!2)
       ((_ is O_internal_node) a!1)))
      )
      (inv_main2408_5_5 C F D A)
    )
  )
)
(assert
  (forall ( (A Int) (B internal_node) (C Heap) (D Int) (E Heap) (F Int) (G Int) (H Heap) (v_8 Int) ) 
    (=>
      (and
        (inv_main2408_5 H G)
        (let ((a!1 (HeapCtor (HeapSize H)
                     (store (HeapContents H)
                            G
                            (O_internal_node (internal_node 0)))))
      (a!3 (HeapCtor (+ 1 (HeapSize E))
                     (store (HeapContents E)
                            (+ 1 (HeapSize E))
                            (O_internal_node B))))
      (a!4 (ite (and (not (<= G 0)) (>= (HeapSize H) G))
                (select (HeapContents H) G)
                defObj)))
(let ((a!2 (ite (and (not (<= G 0)) (>= (HeapSize H) G)) a!1 H)))
  (and (not (= F 0))
       (= D G)
       (= A (+ 1 (HeapSize E)))
       (= E a!2)
       (= C a!3)
       ((_ is O_internal_node) a!4)
       (= v_8 D))))
      )
      (inv_main2408_5_5 C D v_8 A)
    )
  )
)
(assert
  (forall ( (A Heap) (B Int) (C Int) (D Int) (E Heap) ) 
    (=>
      (and
        (inv_main2406_1_4 E D C B)
        (let ((a!1 (HeapCtor (HeapSize E)
                     (store (HeapContents E)
                            C
                            (O_internal_node (internal_node B)))))
      (a!3 (ite (and (not (<= C 0)) (>= (HeapSize E) C))
                (select (HeapContents E) C)
                defObj)))
(let ((a!2 (ite (and (not (<= C 0)) (>= (HeapSize E) C)) a!1 E)))
  (and (= A a!2) ((_ is O_internal_node) a!3))))
      )
      (inv_main_3 A D C)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) (E Int) (F Int) (G Int) (H Heap) ) 
    (=>
      (and
        (inv_main2408_5_5 H G F E)
        (let ((a!1 (HeapCtor (HeapSize H)
                     (store (HeapContents H)
                            E
                            (O_internal_node (internal_node 0)))))
      (a!3 (ite (and (not (<= E 0)) (>= (HeapSize H) E))
                (select (HeapContents H) E)
                defObj)))
(let ((a!2 (ite (and (not (<= E 0)) (>= (HeapSize H) E)) a!1 H)))
  (and (= B F) (= C G) (= A E) (= D a!2) ((_ is O_internal_node) a!3))))
      )
      (inv_main2406_1_4 D C B A)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Int) (F Heap) (G Int) (H Int) (I Int) (J Int) (K Int) (L Heap) ) 
    (=>
      (and
        (inv_main_16 L K J I H G)
        (let ((a!1 (ite (and (not (<= G 0)) (>= (HeapSize L) G))
                (select (HeapContents L) G)
                defObj)))
(let ((a!2 (O_sll (sll (|sll::next| (getsll a!1)) 0))))
(let ((a!3 (ite (and (not (<= G 0)) (>= (HeapSize L) G))
                (HeapCtor (HeapSize L) (store (HeapContents L) G a!2))
                L)))
  (and (= A G) (= E K) (= D J) (= C I) (= B H) (= F a!3) ((_ is O_sll) a!1)))))
      )
      (inv_main2399_1_14 F E D C B A)
    )
  )
)
(assert
  (forall ( (A Heap) (B Int) (C Int) (D Int) (E Heap) ) 
    (=>
      (and
        (inv_main2401_5 E D C B)
        (let ((a!1 (ite (and (not (<= B 0)) (>= (HeapSize E) B))
                (select (HeapContents E) B)
                defObj)))
(let ((a!2 (O_sll (sll 0 (|sll::shared| (getsll a!1))))))
(let ((a!3 (ite (and (not (<= B 0)) (>= (HeapSize E) B))
                (HeapCtor (HeapSize E) (store (HeapContents E) B a!2))
                E)))
  (and (= A a!3) ((_ is O_sll) a!1)))))
      )
      (inv_main_8 A D C B)
    )
  )
)
(assert
  (forall ( (A Heap) (B Int) (C Int) (D Int) (E Int) (F Heap) ) 
    (=>
      (and
        (inv_main_13 F E D C B)
        (let ((a!1 (ite (and (not (<= B 0)) (>= (HeapSize F) B))
                (select (HeapContents F) B)
                defObj)))
(let ((a!2 (not (<= (|sll::next| (getsll a!1)) 0))))
(let ((a!3 (and a!2 (>= (HeapSize F) (|sll::next| (getsll a!1))))))
(let ((a!4 (ite a!3 (select (HeapContents F) (|sll::next| (getsll a!1))) defObj)))
(let ((a!5 (O_sll (sll (|sll::next| (getsll a!4)) D))))
(let ((a!6 (HeapCtor (HeapSize F)
                     (store (HeapContents F) (|sll::next| (getsll a!1)) a!5))))
  (and ((_ is O_sll) a!1) (= A (ite a!3 a!6 F)) ((_ is O_sll) a!4))))))))
      )
      (inv_main_18 A E D C B)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) (E Int) (F Int) (G Int) (H Int) (I Heap) (J Int) (K Int) (L Int) (M Int) (N Heap) ) 
    (=>
      (and
        (inv_main2457_5_20 N M L K)
        (let ((a!1 (ite (and (not (<= K 0)) (>= (HeapSize N) K))
                (select (HeapContents N) K)
                defObj))
      (a!2 (ite (and (not (<= F 0)) (>= (HeapSize I) F))
                (HeapCtor (HeapSize I) (store (HeapContents I) F defObj))
                I)))
  (and (= B G)
       (= H M)
       (= C H)
       (not (= J 0))
       (= J E)
       (= G L)
       (= F K)
       (= E (|internal_node::next| (getinternal_node a!1)))
       (= A F)
       (= D a!2)
       (= I N)
       ((_ is O_internal_node) a!1)))
      )
      (inv_main2457_5_20 D C B J)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) (E Int) (F Int) (G Int) (H Int) (I Int) (J Int) (K Heap) (v_11 Int) ) 
    (=>
      (and
        (inv_main2425_5 K J I H G)
        (let ((a!1 (ite (and (not (<= G 0)) (>= (HeapSize K) G))
                (select (HeapContents K) G)
                defObj)))
(let ((a!2 (O_sll (sll (|sll::next| (getsll a!1)) I))))
(let ((a!3 (ite (and (not (<= G 0)) (>= (HeapSize K) G))
                (HeapCtor (HeapSize K) (store (HeapContents K) G a!2))
                K)))
  (and (= E 0)
       (not (= F 0))
       (= F J)
       (= C I)
       (= B H)
       (= A G)
       (= D a!3)
       ((_ is O_sll) a!1)
       (= v_11 F)))))
      )
      (inv_main2457_5_20 D F B v_11)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Heap) (F Int) (G Int) (H Int) (I Int) (J Int) (K Int) (L Heap) (v_12 Int) ) 
    (=>
      (and
        (inv_main_18 L K J I H)
        (let ((a!1 (ite (and (not (<= H 0)) (>= (HeapSize L) H))
                (select (HeapContents L) H)
                defObj)))
  (and (= F 0)
       (= A (|sll::next| (getsll a!1)))
       (not (= G 0))
       (= G K)
       (= D J)
       (= C I)
       (= B H)
       (= E L)
       ((_ is O_sll) a!1)
       (= v_12 G)))
      )
      (inv_main2457_5_20 E G C v_12)
    )
  )
)
(assert
  (forall ( (A Heap) (B Int) (C Int) (D Int) (E Int) (F Int) (G Heap) ) 
    (=>
      (and
        (inv_main2399_1_14 G F E D C B)
        (let ((a!1 (ite (and (not (<= C 0)) (>= (HeapSize G) C))
                (select (HeapContents G) C)
                defObj)))
(let ((a!2 (O_sll (sll B (|sll::shared| (getsll a!1))))))
(let ((a!3 (ite (and (not (<= C 0)) (>= (HeapSize G) C))
                (HeapCtor (HeapSize G) (store (HeapContents G) C a!2))
                G)))
  (and (= A a!3) ((_ is O_sll) a!1)))))
      )
      (inv_main_13 A F E D C)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) (E Int) (F Int) (G Int) (H Int) (I Heap) (J Int) (K Int) (L Int) (M Int) (N Heap) ) 
    (=>
      (and
        (inv_main2458_5_23 N M L K)
        (let ((a!1 (ite (and (not (<= K 0)) (>= (HeapSize N) K))
                (select (HeapContents N) K)
                defObj))
      (a!2 (ite (and (not (<= F 0)) (>= (HeapSize I) F))
                (HeapCtor (HeapSize I) (store (HeapContents I) F defObj))
                I)))
  (and (= B G)
       (= H M)
       (= C H)
       (not (= J 0))
       (= J E)
       (= G L)
       (= F K)
       (= E (|sll::next| (getsll a!1)))
       (= A F)
       (= D a!2)
       (= I N)
       ((_ is O_sll) a!1)))
      )
      (inv_main2458_5_23 D C B J)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Heap) (D Int) (E Int) (F Int) (G Int) (H Heap) (I Int) (J Int) (K Int) (L Int) (M Int) (N Heap) (v_14 Int) ) 
    (=>
      (and
        (inv_main2457_5_20 N M L K)
        (let ((a!1 (ite (and (not (<= K 0)) (>= (HeapSize N) K))
                (select (HeapContents N) K)
                defObj))
      (a!2 (ite (and (not (<= E 0)) (>= (HeapSize H) E))
                (HeapCtor (HeapSize H) (store (HeapContents H) E defObj))
                H)))
  (and (= B G)
       (not (= J 0))
       (= J F)
       (= I 0)
       (= I D)
       (= G M)
       (= F L)
       (= E K)
       (= D (|internal_node::next| (getinternal_node a!1)))
       (= A E)
       (= H N)
       (= C a!2)
       ((_ is O_internal_node) a!1)
       (= v_14 J)))
      )
      (inv_main2458_5_23 C B J v_14)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Heap) (D Int) (E Int) (F Int) (G Int) (H Int) (I Int) (J Int) (K Heap) (v_11 Int) ) 
    (=>
      (and
        (inv_main2425_5 K J I H G)
        (let ((a!1 (ite (and (not (<= G 0)) (>= (HeapSize K) G))
                (select (HeapContents K) G)
                defObj)))
(let ((a!2 (O_sll (sll (|sll::next| (getsll a!1)) I))))
(let ((a!3 (ite (and (not (<= G 0)) (>= (HeapSize K) G))
                (HeapCtor (HeapSize K) (store (HeapContents K) G a!2))
                K)))
  (and (= E 0)
       (= E J)
       (not (= F 0))
       (= F H)
       (= D 0)
       (= B I)
       (= A G)
       (= C a!3)
       ((_ is O_sll) a!1)
       (= v_11 F)))))
      )
      (inv_main2458_5_23 C E F v_11)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) (E Int) (F Int) (G Int) (H Int) (I Int) (J Int) (K Int) (L Heap) (v_12 Int) ) 
    (=>
      (and
        (inv_main_18 L K J I H)
        (let ((a!1 (ite (and (not (<= H 0)) (>= (HeapSize L) H))
                (select (HeapContents L) H)
                defObj)))
  (and (= F 0)
       (= F K)
       (= A (|sll::next| (getsll a!1)))
       (not (= G 0))
       (= G I)
       (= E 0)
       (= C J)
       (= B H)
       (= D L)
       ((_ is O_sll) a!1)
       (= v_12 G)))
      )
      (inv_main2458_5_23 D F G v_12)
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) (E Int) (F Int) (G Int) (H Heap) (v_8 Int) ) 
    (=>
      (and
        (inv_main_8 H G F E)
        (let ((a!1 (ite (and (not (<= E 0)) (>= (HeapSize H) E))
                (select (HeapContents H) E)
                defObj)))
(let ((a!2 (O_sll (sll (|sll::next| (getsll a!1)) 0))))
(let ((a!3 (ite (and (not (<= E 0)) (>= (HeapSize H) E))
                (HeapCtor (HeapSize H) (store (HeapContents H) E a!2))
                H)))
  (and (= B F) (= C G) (= A E) (= D a!3) ((_ is O_sll) a!1) (= v_8 A)))))
      )
      (inv_main2425_5 D C B A v_8)
    )
  )
)
(assert
  (forall ( (A Int) (B internal_node) (C Heap) (D Heap) ) 
    (=>
      (and
        (inv_main2455_5 D)
        (let ((a!1 (HeapCtor (+ 1 (HeapSize D))
                     (store (HeapContents D)
                            (+ 1 (HeapSize D))
                            (O_internal_node B)))))
  (and (= C a!1) (= A (+ 1 (HeapSize D)))))
      )
      (inv_main2408_5 C A)
    )
  )
)
(assert
  (forall ( (A Int) (B sll) (C Heap) (D Int) (E Int) (F Int) (G Heap) (H Int) (I Int) (J Int) (K Heap) (v_11 Int) ) 
    (=>
      (and
        (inv_main_3 K J I)
        (let ((a!1 (ite (and (not (<= I 0)) (>= (HeapSize K) I))
                (select (HeapContents K) I)
                defObj))
      (a!2 (HeapCtor (+ 1 (HeapSize G))
                     (store (HeapContents G) (+ 1 (HeapSize G)) (O_sll B)))))
  (and (= E I)
       (= H 0)
       (= F J)
       (= D (|internal_node::next| (getinternal_node a!1)))
       (= A (+ 1 (HeapSize G)))
       (= G K)
       (= C a!2)
       ((_ is O_internal_node) a!1)
       (= v_11 F)))
      )
      (inv_main2401_5 C F v_11 A)
    )
  )
)
(assert
  (forall ( (A Int) (B sll) (C Heap) (D Int) (E Heap) (F Int) (G Int) (H Heap) (v_8 Int) ) 
    (=>
      (and
        (inv_main2408_5 H G)
        (let ((a!1 (HeapCtor (HeapSize H)
                     (store (HeapContents H)
                            G
                            (O_internal_node (internal_node 0)))))
      (a!3 (HeapCtor (+ 1 (HeapSize E))
                     (store (HeapContents E) (+ 1 (HeapSize E)) (O_sll B))))
      (a!4 (ite (and (not (<= G 0)) (>= (HeapSize H) G))
                (select (HeapContents H) G)
                defObj)))
(let ((a!2 (ite (and (not (<= G 0)) (>= (HeapSize H) G)) a!1 H)))
  (and (= F 0)
       (= D G)
       (= A (+ 1 (HeapSize E)))
       (= E a!2)
       (= C a!3)
       ((_ is O_internal_node) a!4)
       (= v_8 D))))
      )
      (inv_main2401_5 C D v_8 A)
    )
  )
)
(assert
  (forall ( (A Heap) (B Int) (C Int) (D Int) (E Int) (F Int) (G Heap) ) 
    (=>
      (and
        (inv_main2401_5_15 G F E D C B)
        (let ((a!1 (ite (and (not (<= B 0)) (>= (HeapSize G) B))
                (select (HeapContents G) B)
                defObj)))
(let ((a!2 (O_sll (sll 0 (|sll::shared| (getsll a!1))))))
(let ((a!3 (ite (and (not (<= B 0)) (>= (HeapSize G) B))
                (HeapCtor (HeapSize G) (store (HeapContents G) B a!2))
                G)))
  (and (= A a!3) ((_ is O_sll) a!1)))))
      )
      (inv_main_16 A F E D C B)
    )
  )
)
(assert
  (forall ( (A Int) (B Heap) ) 
    (=>
      (and
        (inv_main2408_5 B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize B) A))
                (select (HeapContents B) A)
                defObj)))
  (not ((_ is O_internal_node) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) ) 
    (=>
      (and
        (inv_main2408_5_5 D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize D) A))
                (select (HeapContents D) A)
                defObj)))
  (not ((_ is O_internal_node) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) ) 
    (=>
      (and
        (inv_main2406_1_4 D C B A)
        (let ((a!1 (ite (and (not (<= B 0)) (>= (HeapSize D) B))
                (select (HeapContents D) B)
                defObj)))
  (not ((_ is O_internal_node) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Heap) ) 
    (=>
      (and
        (inv_main_3 C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize C) A))
                (select (HeapContents C) A)
                defObj)))
  (not ((_ is O_internal_node) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) ) 
    (=>
      (and
        (inv_main2401_5 D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize D) A))
                (select (HeapContents D) A)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) ) 
    (=>
      (and
        (inv_main_8 D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize D) A))
                (select (HeapContents D) A)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Heap) ) 
    (=>
      (and
        (inv_main2425_5 E D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize E) A))
                (select (HeapContents E) A)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Int) (F Heap) ) 
    (=>
      (and
        (inv_main2401_5_15 F E D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize F) A))
                (select (HeapContents F) A)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Int) (F Heap) ) 
    (=>
      (and
        (inv_main_16 F E D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize F) A))
                (select (HeapContents F) A)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Int) (F Heap) ) 
    (=>
      (and
        (inv_main2399_1_14 F E D C B A)
        (let ((a!1 (ite (and (not (<= B 0)) (>= (HeapSize F) B))
                (select (HeapContents F) B)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Heap) ) 
    (=>
      (and
        (inv_main_13 E D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize E) A))
                (select (HeapContents E) A)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Heap) ) 
    (=>
      (and
        (inv_main_13 E D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize E) A))
                (select (HeapContents E) A)
                defObj)))
(let ((a!2 (not (<= (|sll::next| (getsll a!1)) 0))))
(let ((a!3 (and a!2 (>= (HeapSize E) (|sll::next| (getsll a!1))))))
(let ((a!4 (ite a!3 (select (HeapContents E) (|sll::next| (getsll a!1))) defObj)))
  (and ((_ is O_sll) a!1) (not ((_ is O_sll) a!4)))))))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Int) (E Heap) ) 
    (=>
      (and
        (inv_main_18 E D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize E) A))
                (select (HeapContents E) A)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) ) 
    (=>
      (and
        (inv_main2457_5_20 D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize D) A))
                (select (HeapContents D) A)
                defObj)))
  (not ((_ is O_internal_node) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (A Int) (B Int) (C Int) (D Heap) ) 
    (=>
      (and
        (inv_main2458_5_23 D C B A)
        (let ((a!1 (ite (and (not (<= A 0)) (>= (HeapSize D) A))
                (select (HeapContents D) A)
                defObj)))
  (not ((_ is O_sll) a!1)))
      )
      CHC_COMP_FALSE
    )
  )
)
(assert
  (forall ( (CHC_COMP_UNUSED Bool) ) 
    (=>
      (and
        CHC_COMP_FALSE
      )
      false
    )
  )
)

(check-sat)
(exit)
